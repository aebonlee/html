...
- **스크롤바 숨김**: 가로 스크롤 기능은 유지하되 시각적 스크롤바 숨김 (`scrollbar-none`)
- **하위 페이지 sticky**: 모든 뷰에서 헤더 `sticky top-0 z-20` 고정, 사이드바/드롭다운 `z-10` 안정화
...
- **우측 코드 패널** (`w-[580px]`, lg+ 데스크톱): 4-Tab 코드 예시
...
| 화면 크기 | 그리드 뷰 | 상세 뷰 |
|-----------|----------|----------|
| ~640px (모바일) | 그룹 탭 + 칩 + 2열 카드 | 드롭다운 2개 + 전체 너비 콘텐츠 |
| 640px~ (sm) | 그룹 탭 + 칩 + 3열 카드 | 드롭다운 2개 + 전체 너비 콘텐츠 |
| 768px~ (md) | 헤더 풍선 드롭다운 메뉴 + auto-fill 카드 | 좌측 아코디언 사이드바 + 중앙 콘텐츠 + 하단 코드 접이식 |
| 1024px~ (lg) | 헤더 풍선 드롭다운 메뉴 + auto-fill 카드 | 좌측 아코디언 사이드바 + 중앙 콘텐츠 + 우측 코드 패널(w-[580px]) |
