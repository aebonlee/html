...
### Phase 6 - 상세 뷰 3-Panel 레이아웃 개편 (2026-02-27)

**주요 변경 사항:**

#### 상세 뷰 레이아웃 전면 재구성 (카테고리 탭 + 드롭다운 → 3-Panel)
- **기존**: 상단 카테고리 탭(가로 스크롤 칩) + 용어 드롭다운 + 콘텐츠가 세로로 쌓인 단일 컬럼 구조
- **변경**: 데스크톱(md+)에서 좌/중/우 3-Panel 레이아웃으로 전환
  - **좌측 사이드바** (`w-48`, sticky): 13개 카테고리 세로 목록 (매크로 내비게이션)
  - **중앙 콘텐츠** (`flex-1`): 선택된 용어의 상세 정보 (설명/사용법/예시/SVG 시각화)
  - **우측 사이드바** (`w-52`, sticky): 현재 카테고리의 용어 목록 (마이크로 내비게이션)
- **모바일**(<md): 좌/우 사이드바 대신 2개의 `<select>` 드롭다운으로 대체
  - 카테고리 드롭다운 + 용어 드롭다운
- 사이드바는 `sticky top-14 h-[calc(100vh-3.5rem)]`로 헤더 아래 고정
- 선택된 항목: `bg-starbucks-green text-white` 하이라이트

#### 이벤트 핸들러 업데이트
- `.category-btn` 클릭 → 카테고리 전환 + 해당 카테고리 첫 용어 자동 선택
- `.term-btn` 클릭 → 용어 전환 + 중앙 콘텐츠 갱신
- `#mobileCategoryDropdown` change → 모바일 카테고리 전환
- `#mobileTermDropdown` change → 모바일 용어 전환
- 기존 `.category-tab` 및 `#termDropdown` 핸들러 제거
...
### 4.2 상세 뷰 (3-Panel 레이아웃)

**데스크톱 (md+):**
- **상단 헤더**: 홈 버튼, 제목, 언어/테마 토글 (sticky)
- **좌측 사이드바** (`w-48`, sticky): 13개 카테고리 세로 목록
- **중앙 콘텐츠** (`flex-1`): 선택된 용어의 4가지 정보 표시
  - 설명 (Description)
  - 언제 써요? (When to Use?)
  - 예시 (Example)
  - 시각화 예시 (Visual Example) - SVG 렌더링
- **우측 사이드바** (`w-52`, sticky): 현재 카테고리의 용어 버튼 목록

**모바일 (<md):**
- 좌/우 사이드바 대신 2개의 `<select>` 드롭다운 표시
- 콘텐츠 영역 전체 너비 활용
...
| 768px~ | 3열 | 3-Panel (좌 카테고리 + 중앙 콘텐츠 + 우 용어 목록) |
| 1024px~ | 4열 | 3-Panel (좌 카테고리 + 중앙 콘텐츠 + 우 용어 목록) |
...
| 2026-02-27 | - | 상세 뷰 3-Panel 레이아웃 개편 (좌 카테고리 + 중앙 콘텐츠 + 우 용어 목록), 모바일 드롭다운 대응 |
