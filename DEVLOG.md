...
### Phase 17 - 하위 페이지 탑메뉴 + 기존 좌측 메뉴 복원 (2026-02-27)

**주요 변경 사항:**

#### 탑메뉴 복원
- **상세 뷰 / Vibe Coding 뷰**: `renderMobileSubHeader()` + `renderSideNav()` 조합을 제거하고 기존 `renderHeader()` 수평 헤더로 복원
- 모든 뷰에서 동일한 탑메뉴 표시: `로고 | Vibe Coding | Web Programming | UI 기본▾ | UI 심화▾ | UI 심화확장▾ | Q&A | 교육신청 | 🔍 | EN | 🌙`

#### 기존 좌측 사이드바 복원
- **상세 뷰**: UI 기본부터 시작하는 기존 아코디언 사이드바(`buildSidebarHtml()`) 복원
  - `sticky top-14 h-[calc(100vh-3.5rem)]` (헤더 아래 고정)
- **우측 코드 패널**: `sticky top-14 h-[calc(100vh-3.5rem)]` 복원
- **Vibe Coding 뷰**: 사이드바 없이 탑메뉴 + 탭바 + 콘텐츠 구조 복원

#### 결과 레이아웃
| 뷰 | 데스크톱 | 모바일 |
|------|----------|--------|
| **그리드 뷰** | 탑메뉴 + 그룹탭 + 카드 | 탑메뉴 + 햄버거 + 그룹탭 + 칩 |
