...
UI/UX 디자인에서 자주 사용되는 71개 용어를 **한국어/영어** 양국어로 제공하는 인터랙티브 웹 용어집을 구축한다. 각 용어에 대해 설명, 사용 시점, 실제 예시, SVG 시각화 예시를 포함하여 비전공자도 직관적으로 이해할 수 있도록 한다.
...
- **사이드바**: 카테고리별 용어 목록, 검색 필터, 홈 버튼, 언어/테마 토글
- **콘텐츠 패널**: 선택된 용어의 4가지 정보 표시
  - 설명 (Description)
  - 언제 써요? (When to Use?)
  - 예시 (Example)
...
## 5. 용어 카테고리 (13개, 총 71개 용어)

| # | 카테고리 | 용어 수 |
|---|---------|--------|
| 1 | UI 기본 - 1) 입력·선택 컴포넌트 | 8 |
| 2 | UI 기본 - 2) 내비게이션·구조 | 9 |
| 3 | UI 기본 - 3) 피드백·상태 표시 | 10 |
| 4 | UI 기본 - 4) 레이아웃·표시 컴포넌트 | 8 |
| 5 | UI 기본 - 5) 상호작용 패턴·상태 | 6 |
| 6 | UI 기본 - 6) 폼 도움 요소 | 6 |
| 7 | UI 심화 - 1) 내비게이션 & 레이아웃 | 3 |
| 8 | UI 심화 - 2) 상호작용 & 피드백 | 3 |
| 9 | UI 심화 - 3) 데이터 표현 & 시각화 | 2 |
| 10 | UI 심화 - 4) 고급 입력 컴포넌트 | 3 |
...
| **SVG** | 시각화 예시 | 벡터 기반으로 다크모드 대응, 애니메이션 내장 |
...
4. **SVG 인라인 렌더링**: 71개 시각화 예시를 SVG로 직접 렌더링하여 외부 이미지 의존성 제거
...
### 우선순위 높음
- [ ] 카테고리별 필터 버튼 추가 (그리드 뷰 상단에 카테고리 칩 배치)
- [ ] 검색 시 매칭 텍스트 하이라이팅
