...
### 4.2 상세 뷰 (카테고리 탭 + 드롭다운)

- **상단 헤더**: 홈 버튼, 제목, 언어/테마 토글 (sticky)
- **카테고리 탭 바**: 가로 스크롤 칩 버튼으로 카테고리 전환 (sticky)
- **용어 드롭다운**: 선택된 카테고리의 용어를 `<select>`로 선택
- **콘텐츠 패널**: 선택된 용어의 4가지 정보 표시 (전체 너비 활용)
  - 설명 (Description)
  - 언제 써요? (When to Use?)
  - 예시 (Example)
...
| 화면 크기 | 그리드 열 | 상세 뷰 |
|-----------|----------|----------|
| ~640px | 1열 | 카테고리 탭 가로 스크롤 |
| 640px~ | 2열 | 카테고리 탭 가로 스크롤 |
| 768px~ | 3열 | 탭 + 드롭다운 + 전체 너비 콘텐츠 |
| 1024px~ | 4열 | 탭 + 드롭다운 + 전체 너비 콘텐츠 |
...
### 우선순위 높음
- [x] ~~카테고리별 필터 버튼 추가~~ → 상세 뷰에 카테고리 탭 바로 구현 완료
- [ ] 검색 시 매칭 텍스트 하이라이팅
